...
// љbersetzung: Helmut Knappe, dc soft
// *****************************************************************************
//------------------------------------------------------------
//
//Funktion:
...
//Zweck:
//
//     Gibt die Meldungen der Anwendung zurЃck
//
//Syntax:
//
//     dfStdMsg1( <nMsg> ) --> cMsg
...
//     zurЃck. Sie k”nnen auch die in Visual dBsee  definierten Konstanten verwenden.
...
//     sollen, Ѓbersetzen Sie diese Datei und kompilieren sie.
//
//     Dies haben wir fЃr Sie bereits gemacht. Aber falls Sie noch Ћnderungen
//     anbringen m”chten mЃssen Sie so verfahren (dc soft GmbH).
//
//     Sie sollten eine Backup-Kopie der Datei anlegen, bevor Sie Ћnderungen
//     durchfЃhren. Beachten Sie, daб ein Upgrade oder Update diese Datei wieder
//     Ѓberschreiben kann.
//
//     Kompilieren Sie das Programm mit: XPP deutsch -n [-l -m -w]
//
//     Damit das System die neuen Meldungen erkennt, fЃgen Sie diese Datei zur
//     Liste der Externals in Ihrem Projekt hinzu, damit das Objekt mit
//     eingebunden wird. Falls sie die Ћnderungen festschreiben wollen,
...
//
//     #include "dfMsg.ch"
//     ? dfStdMsg( MSG_DE_STATE_ADD ) // "EinfЃgen"
//     // Zugeh”rige Meldung zurЃckgeben
...
DO CASE
   ...
   CASE nMsg == MSG1_LANGUAGE       ; cMsg := "DEUTSCH"
   // ДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДД
   // ДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДДД
