...
CLASS S2PrintDispCRWSMTPMail FROM S2PrintDisp
   PROTECTED:
      METHOD createFile
...
METHOD S2PrintDispCRWSMTPMail:init( oParent, oOwner, aPos, aSize, aPP, lVisible )
   LOCAL oXbp
...
METHOD S2PrintDispCRWSMTPMail:create( oParent, oOwner, aPos, aSize, aPP, lVisible )
...
METHOD S2PrintDispCRWSMTPMail:exitMenu(nAction, aBuf)
...
METHOD S2PrintDispCRWSMTPMail:execute()
   LOCAL oMail
   LOCAL lRet := .F.
...
METHOD S2PrintDispCRWSMTPMail:OutPutSelect()
   LOCAL cFile, cPath
...
METHOD S2PrintDispCRWSMTPMail:createFile() // cFile, cTitle, xDevice)
   LOCAL lRet      := .T.
...
// DA ATTIVARE
// METHOD S2PrintDispCRWSMTPMail:rubrica()
//    LOCAL xRet := ""
//
//    dfAlert("da fare...")
...
METHOD S2PrintDispCRWSMTPMail:zipFile(cFileName)
   LOCAL cRet
...
METHOD S2PrintDispCRWSMTPMail:getExportTypes()
...
METHOD S2PrintDispCRWSMTPMail:Close()
   LOCAL lRet      := .F.
   LOCAL oRT       := ::aBuffer[REP_XBASEREPORTTYPE]
   LOCAL oCRWOut   := oRT:getOutput()
