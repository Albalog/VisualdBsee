...
CLASS S2PrintDispMAPIMail FROM S2PrintDisp
   EXPORTED:
      ...
      VAR cMailTo,cOggetto,cBody,cFrom,cReplyTo
      VAR cAutoCaption
...
METHOD S2PrintDispMAPIMail:init( oParent, oOwner, aPos, aSize, aPP, lVisible )
   LOCAL oXbp
...
METHOD S2PrintDispMAPIMail:create( oParent, oOwner, aPos, aSize, aPP, lVisible )
...
METHOD S2PrintDispMAPIMail:exitMenu(nAction, aBuf)
...
METHOD S2PrintDispMapiMail:execute()
   LOCAL oMail
   LOCAL lRet := .F.
...
// DA ATTIVARE
// METHOD S2PrintDispMAPIMail:rubrica()
//    LOCAL xRet := ""
//
//    dfAlert("da fare...")
...
METHOD S2PrintDispMAPIMail:OutPutSelect( n )
   LOCAL cFile, cPath
...
METHOD S2PrintDispMAPIMail:End_Execute(cFilePdf,lOk,cFileTxt,cFile,oMail,nOptions)
   IF lOk
      // Simone 05/set/03 gerr 3926
      IF ::lZip 
      ...
      FERASE(cFile)
      IF nOptions >=2
         dfalert(dfStdMsg1(MSG1_S2PDISMM08))
      ENDIF
   ELSE
      //Errore creazione File Pdf
      dbMsgErr(dfStdMsg1(MSG1_S2PDISFI08))
...
METHOD S2PrintDispMAPIMail:zipFile(cFileName)
   LOCAL cRet
